...
namespace biliqt::core::api {
    template<typename T>
    std::shared_ptr<T> createShared(const std::string &baseUrl) {
    ...
    doSign(T* body, std::string apiSecret) {
        ...
        while (!keys.empty()) {
            ...
            std::string strVal = utils::to_string(value.get());
            oss << key << "=" << strVal;
        }
        oss << apiSecret;
        ...
        for (const auto &field: map) {
            ...
            auto value = map[field.second]->get(body);
            std::string strVal = utils::to_string(value.get());
...
#define BILI_SIGN_CLIENT_INIT(Class, BASE_URL)                                                     \
    API_CLIENT_INIT(Class)                                                                         \
public:                                                                                            \
    static std::shared_ptr<Class> createShared() {                                                 \
        return biliqt::core::api::createShared<Class>(BASE_URL);                                   \
    }
